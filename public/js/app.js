(function () {
  ...
  const COUPONS_ENDPOINT = "/.netlify/functions/coupons";
  ...
  const cartTotalsBox = document.getElementById("cartTotalsBox");
  ...
  async function applyCoupon() {
    ...
    try {
      ...
      if (!data.valid) {
        ...
        couponMsg.textContent = "Ese cupón no existe o ya fue usado.";
        renderTotals();
        return;
      }

      appliedCoupon = { code: code.trim().toUpperCase(), type: data.type, value: data.value };
      couponMsg.hidden = false;
      couponMsg.className = "coupon-msg ok";
      const benefitText =
        data.type === "percent" ? `${data.value}% de descuento` : `${formatPrice(data.value)} de descuento`;
      couponMsg.textContent = `Cupón aplicado: ${benefitText}${data.description ? " — " + data.description : ""}`;
      renderTotals();
    } catch (e) {
      couponMsg.hidden = false;
      couponMsg.className = "coupon-msg error";
      couponMsg.textContent = "No se pudo validar el cupón. Probá de nuevo.";
    } finally {
  ...
  couponInput.addEventListener("input", () => {
    if (appliedCoupon && couponInput.value.trim().toUpperCase() !== appliedCoupon.code) {
      appliedCoupon = null;
  ...
  function renderTotals() {
    ...
    let html = "";
    if (appliedCoupon) {
      html += `<div class="totals-row"><span>Subtotal</span><span>${formatPrice(subtotal)}</span></div>`;
      html += `<div class="totals-row discount"><span>Descuento (${appliedCoupon.code})</span><span>-${formatPrice(discount)}</span></div>`;
    }
    html += `<div class="totals-row final"><span>Total</span><span>${formatPrice(subtotal - discount)}</span></div>`;
    cartTotalsBox.innerHTML = html;
  }
  ...
  function buildWhatsappMessage() {
    ...
    if (appliedCoupon && discount > 0) {
      ...
      lines.push(`Cupón ${appliedCoupon.code}: -${formatPrice(discount)}`);
      lines.push(`Total con descuento: ${formatPrice(subtotal - discount)}`);
    } else {
  ...
  sendOrderBtn.addEventListener("click", async () => {
    if (appliedCoupon) {
      ...
      try {
        ...
        const data = await res.json();
        if (!data.redeemed) {
          // Alguien más lo usó justo antes, o ya no es válido.
          appliedCoupon = null;
          couponMsg.hidden = false;
          couponMsg.className = "coupon-msg error";
          couponMsg.textContent = "Ese cupón ya no está disponible. Se quitó del pedido.";
          renderTotals();
          sendOrderBtn.disabled = false;
          sendOrderBtn.textContent = "Enviar pedido por WhatsApp";
          return;
        }
      } catch (e) {
        // Si falla la conexión, igual dejamos mandar el pedido sin descuento aplicado en el servidor.
      }
      sendOrderBtn.disabled = false;
      sendOrderBtn.textContent = "Enviar pedido por WhatsApp";
    }

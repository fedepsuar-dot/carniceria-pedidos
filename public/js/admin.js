(function () {
  ...
  couponTypeSelect.addEventListener("change", () => {
    couponValueLabel.textContent = couponTypeSelect.value === "percent" ? "Valor (%)" : "Valor ($)";
  });
  ...
  async function saveCoupon(coupon) {
    const res = await fetch(COUPONS_ENDPOINT, {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-admin-token": getToken() },
      body: JSON.stringify({ action: "upsert", coupon }),
    });
    if (res.status === 401) throw new Error("unauthorized");
    if (!res.ok) throw new Error("save failed");
    return res.json();
  ...
  function formatCouponBenefit(c) {
    return c.type === "percent" ? `${c.value}% de descuento` : `${formatPriceAdmin(c.value)} de descuento`;
  }

  function renderCouponList() {
    if (coupons.length === 0) {
      couponListEl.innerHTML = '<p class="admin-hint">No hay cupones activos por el momento.</p>';
      return;
    }
    ...
    couponListEl.innerHTML = coupons
      .map(
        (c) => `
          <div class="admin-product-row" data-code="${escapeAttr(c.code)}">
            <div class="combo-row-summary">
              <strong>${escapeAttr(c.code)}</strong> — ${formatCouponBenefit(c)}
              ${c.description ? `<br>${escapeAttr(c.description)}` : ""}
            </div>
            <div class="row-actions">
              <button type="button" class="row-delete-coupon">Borrar</button>
            </div>
          </div>
        `
      )
    ...
    couponListEl.querySelectorAll(".admin-product-row").forEach((row) => {
      const code = row.dataset.code;
      row.querySelector(".row-delete-coupon").addEventListener("click", async () => {
  ...
  couponForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      renderCouponList();
      couponForm.reset();
      couponValueLabel.textContent = "Valor (%)";
    } catch (err) {
      handleAuthError(err);
    }
  });
